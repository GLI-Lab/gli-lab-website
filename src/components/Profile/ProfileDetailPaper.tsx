...
import { getPaperTitleId, getPaperTitleKey, isLocalizedPaperTitle } from '@/lib/utils';
...
function splitPapersByScope(papers: PaperData[]) {
    return {
        international: papers.filter(
            (paper) => paper.status !== 'Accepted' || paper.filter.scope === 'international',
        ),
        domestic: papers.filter(
            (paper) => paper.status === 'Accepted' && paper.filter.scope === 'domestic',
        ),
    };
}

function ProfilePaperItem({
    paper,
    index,
    profileYamlId,
}: {
    ...
    return (
        <div className="mb-3 md:mb-4 leading-snug">
                ...
                <div className="flex items-start">
                    ...
                    <div className="flex-1">
                        <div className="text-[15.5px] md:text-[16.5px] font-medium mb-1">
                            <Link
                                href={`/publications/papers#${encodeURIComponent(getPaperTitleId(paper.title))}`}
                                className="hover:text-interactive-hover hover:underline underline-offset-4"
                                title="View publication details"
                            >
                                {isLocalizedPaperTitle(paper.title) ? (
                                    <>
                                        {paper.title.ko}
                                        <br />
...
function ProfilePaperScopeSection({
    title,
    papers,
    profileYamlId,
}: {
        ...
        <div className="grid gap-x-4 gap-y-2">
            ...
            <div>
                {visiblePapers.map((paper, index) => (
                    <ProfilePaperItem
                        key={getPaperTitleKey(paper.title)}
                        paper={paper}
                        index={index}
                        profileYamlId={profileYamlId}
                    />
                ))}
                {(showMore || showLess) && (
...
export function ProfileDetailPaper({ papers, profileYamlId }: ProfileDetailPaperProps) {
    ...
    const { international, domestic } = splitPapersByScope(papers);

    return (
        <>
            {international.length > 0 && (
                <ProfilePaperScopeSection
                    title="Papers (International)"
